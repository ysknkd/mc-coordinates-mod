...
public final class PlayerIndicatorRenderer implements HudRenderCallback {
    ...
    public void onHudRender(DrawContext context, RenderTickCounter tickCounter) {
        ...
        for (PlayerLocationEntity location : PlayerLocationCache.getLocations()) {
            // ワールド座標をブロック中央として扱う
            float worldX = (float)(Math.floor(location.x) + 0.5);
            float worldY = (float)(Math.floor(location.y) + 0.5);
            float worldZ = (float)(Math.floor(location.z) + 0.5);
            ...
            GameProfile profile = Util.getGameProfileByUuid(client, location.uuid);
            if (profile == null) {
